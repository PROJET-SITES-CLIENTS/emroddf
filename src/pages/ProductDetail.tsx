      ...
      <div className="container mx-auto max-w-7xl px-6 md:px-12">
        ...
        <div className="grid lg:grid-cols-12 gap-12 lg:gap-16 items-start">
          ...
          <div className="lg:col-span-7 flex flex-col gap-4 lg:sticky lg:top-32">
            {/* Main image */}
            <div 
              className="relative bg-secondary/30 aspect-square md:aspect-[4/3] lg:aspect-[4/3] lg:max-h-[500px] overflow-hidden group rounded-sm cursor-pointer border border-border/50"
              ...
              {product.images && product.images.length > 0 ? (
                <>
                  ...
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors duration-300 flex items-center justify-center">
                    <div className="bg-white/20 backdrop-blur-md p-4 rounded-sm opacity-0 group-hover:opacity-100 transform scale-50 group-hover:scale-100 transition-all duration-300 shadow-xl">
                  ...
                  {product.images.length > 1 && (
                    <>
                      <button
                        ...
                        className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/10 backdrop-blur-md border border-white/20 p-3 rounded-sm text-white hover:bg-white hover:text-black transition-all opacity-0 group-hover:opacity-100 shadow-lg"
                      >
                      ...
                      <button
                        ...
                        className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/10 backdrop-blur-md border border-white/20 p-3 rounded-sm text-white hover:bg-white hover:text-black transition-all opacity-0 group-hover:opacity-100 shadow-lg"
                      >
              ...
              <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide snap-x">
                {product.images.map((img, i) => (
                  <button
                    ...
                    className={`relative flex-shrink-0 w-20 h-20 md:w-24 md:h-24 overflow-hidden rounded-sm snap-start transition-all duration-300 ${
                      i === currentIndex
          ...
          >
            {/* Category */}
            <div className="text-[10px] uppercase tracking-[0.3em] font-medium text-accent mb-4 flex items-center gap-2">
            ...
            {displayDescription && (
              <div className="mb-8">
                <h3 className="text-xs uppercase tracking-[0.2em] font-medium text-foreground/50 mb-3">À propos de ce modèle</h3>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-10">
              {[
                ...
                <div key={label} className="flex flex-col gap-1.5 p-4 bg-secondary/30 rounded-sm border border-border/50">
            ...
            <div className="flex flex-col gap-4 mt-auto">
              <motion.button
                ...
                className="w-full bg-primary text-primary-foreground uppercase tracking-[0.2em] text-xs font-medium py-5 flex items-center justify-center gap-3 hover:bg-primary/90 transition-colors rounded-sm"
              >
        ...
        <AnimatePresence>
          {isLightboxOpen && (
            ...
            >
              <button
                onClick={() => setIsLightboxOpen(false)}
                className="absolute top-6 md:top-8 right-6 md:right-8 text-white/50 hover:text-white z-50 bg-white/5 hover:bg-white/10 border border-white/10 p-3 md:p-4 rounded-sm transition-all duration-300"
              >
              ...
              {product.images.length > 1 && (
                <>
                  <button
                    onClick={(e) => { e.stopPropagation(); prev(); }}
                    className="absolute left-4 md:left-12 top-1/2 -translate-y-1/2 text-white/50 hover:text-white z-50 bg-white/5 hover:bg-white/10 border border-white/10 p-3 md:p-5 rounded-sm transition-all duration-300"
                  >
                    <ChevronLeft className="w-6 h-6 md:w-8 md:h-8" />
                  </button>
                  <button
                    onClick={(e) => { e.stopPropagation(); next(); }}
                    className="absolute right-4 md:right-12 top-1/2 -translate-y-1/2 text-white/50 hover:text-white z-50 bg-white/5 hover:bg-white/10 border border-white/10 p-3 md:p-5 rounded-sm transition-all duration-300"
                  >
              ...
              {product.images.length > 1 && (
                <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-2 z-50">
                  {product.images.map((_, idx) => (
                    <div 
                      key={idx} 
                      className={`h-1.5 rounded-sm transition-all duration-300 ${idx === currentIndex ? 'w-8 bg-accent' : 'w-2 bg-white/20'}`}
                    />
                  ))}
                </div>
              )}
