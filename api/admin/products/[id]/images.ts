// ══════════════════════════════════════════════════════════════════
// POST /api/admin/products/[id]/images — ajoute une image (URL Blob)
//   { url }
// PUT /api/admin/products/[id]/images — réordonne / définit principale
//   { order: [imageId,...], mainImageId? }
// ══════════════════════════════════════════════════════════════════
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const [{ count }] = await sql`
        SELECT COUNT(*)::int AS count FROM product_images WHERE product_id = ${productId}
      `;
      const isMain = count === 0; // première image = principale par défaut

      const [row] = await sql`
        INSERT INTO product_images (product_id, url, is_main, position)
        VALUES (${productId}, ${url.slice(0, 1000)}, ${isMain}, ${count})
        RETURNING id, url, is_main, position
      `;
    ...
    return res.status(500).json({ error: "Erreur images du produit" });
  }
}
